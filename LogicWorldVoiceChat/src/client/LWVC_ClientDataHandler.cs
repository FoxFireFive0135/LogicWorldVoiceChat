...
namespace FoxFireFive_LogicWorldVoiceChat.Client
{
    public class LWVC_ClientDataHandler : PacketHandler<LWVC_MicrophoneData>
    {
        ...
        {
            if (!LogicWorldVoiceChat.players.TryGetValue(packet.sender, out var aud) || aud == null)
            {
                ...
                aud_src.rolloffMode = AudioRolloffMode.Logarithmic;
                aud_src.minDistance = 1f;
                aud_src.maxDistance = 50f;
            ...
            if (aud == null)
            {
                LogicWorldVoiceChat.players.Remove(packet.sender);
            }
